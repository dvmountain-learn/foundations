...
function changeNums(array, operator, amount) {
    let newArray = array.map((element, index, array) => {
        if (operator === 'add') return element + amount
        else if (operator === 'subtract') return element - amount
        else if (operator === 'multiply') return element * amount
        else if (operator === 'divide') return element / amount
    })
    return newArray
}
...
//  the returned array.
let resultArray = changeNums([1, 2, 3], 'multiply', 5)
// array will change [[6, 7, 8]]
