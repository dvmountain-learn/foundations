...
const findGrapeAsExpression = function(arr) {
    for(let i=0; i<arr.length; i++) {
        if(arr[i].color === "purple") {
            return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a grape`)
        } 
...
// CODE HERE
// function printParam(param1, param2) {
//     console.log(`The first parameter is [${param1}]. The second parameter is [${param2}]`)
// }
...
greeting('Senghort', 'Kheang', (fullName) => {
...
// CODE HERE
function pricesPlusTax(arrays, callback) {
    for (let i = 0; i < arrays.length; i++) {
        // try to convert 20% -> 0.2
        totalCost.push(arrays[i] + 0.2)
    }
    callback(totalCost)
}
...
pricesPlusTax(prices, totalCost => {
    console.log(`The new array push tax: ${totalCost}`)
})
...
function multiplyingFactory(number) {
    return function(anotherNumber) {
        if (number >= 5) {
            ...
            console.log('Cannot multiply: the first number is smaller then 5.')
        }
    }
}
...
// CODE HERE
let timesFour = multiplyingFactory(4)
/* 
