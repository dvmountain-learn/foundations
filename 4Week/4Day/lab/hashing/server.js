...
app.post("/login", (req, res) => {
  ...
  for (let i = 0; i < keys.length; i++) {
    console.log(db[keys[i]].password);
    if (db[keys[i]].password === password) {
      console.log(`The user in by storing their ${req.session.username} in the session.`)
      req.session.success = 'Success';
      res.redirect('/login/success');
      return
    }
  }
  // res.status(400).send({error: `The password doesn't match.`})
  req.session.error = `The password doesn't match.`;
});
...
app.post("/register", (req, res) => {
  ...
  // console.log(db, keys)
  for (let i = 0; i < keys.length; i++) {
    if (db[keys[i]].username === username) {
      req.session.error = 'Username already exists in the database.';
    } else {
      req.session.success = 'Success';
      return
    }
    console.log(db[keys[i]], username)
    if (db[keys[i]] === username) {
