...
//     axios.get(`${baseURL}/say-hello`).then((res) => {}).catch((error) => {})
// })
...
function changeBackgroundColorWithMouseOver() {
    sayHelloButton.style.background = 'black'
    sayHelloButton.style.color = 'white'
}
sayHelloButton.addEventListener('mouseover', changeBackgroundColorWithMouseOver)
...
// CODE HERE
function changeBackgroundColorWithMouseOut() {
    sayHelloButton.style.background = '#EFEFEF'
    sayHelloButton.style.color = 'black'
}
sayHelloButton.addEventListener('mouseout', changeBackgroundColorWithMouseOut)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`${baseURL}/repeat/sokchan`)
        .then((res) => {
...
let repeatButton = document.getElementById('repeat-button').addEventListener('click', repeatMyParam)
...
// CODE HERE
function fetchData() {
    axios.get(`${baseURL}/query-test?search=cat`)
        .then((res) => {
            console.log(res.data)
        })
        .catch((error) => {
...
// document.getElementById('query-button').addEventListener('click', fetchData)
...
document.getElementById('animals-button').addEventListener('click', ohMy2)
...
// 1) If I try to send no queries, it will be get a !
// 2) If I try to send with 1 or 2 queries, it will be get a value that you try to pass (static)
function fetchData2() {
    axios.get(`${baseURL}/query-test?search=dog`)
        .then((res) => {
            console.log(res.data)
        })
        .catch((error) => {
...
document.getElementById('query-button').addEventListener('click', fetchData2)
